...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=5, device='cpu'):
            ...
            for inputs, labels in tqdm(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    if phase == 'val':
                        # 在验证阶段进行三次随机裁剪
                        preds_list = []
                        for _ in range(3):  # 随机裁剪三次
                            outputs = model(inputs)
                            preds_list.append(outputs)

                        # 将三次预测结果进行平均
                        outputs = torch.mean(torch.stack(preds_list), dim=0)
                    else:
                        outputs = model(inputs)
...
def main():
    ...
    # 保存模型权重
    save_path = os.path.join(os.path.dirname(__file__), 'resnet18-finetuned101.pth')
